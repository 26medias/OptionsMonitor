(function(window) {
	...
	window.appSettings.module.directive('uiHome', ['$compile', '$timeout', function ($compile, $timeout) {
		var component = function($scope, element, attrs, ctlr, transcludeFn) {
			...
			$scope.nav = {
				...
				goto:	function(p, requireAuth) {
					if (!requireAuth) {
						document.location.hash = p;
						return true;
					}
					auth.auth(function(success) {
						if (success) {
							document.location.hash = p;
						}
					})
		...
		return {
			...
			templateUrl:	window.appSettings.path.components+'/static/home.html'
		};
	}]);
})(window);