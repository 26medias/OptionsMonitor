...
class Options {
    ...
    /**
     * Converts a JS Date object to "YYYYMMDD" format.
     * @param {Date} dateObj
     * @returns {string}
     */
    ...
    async getContractData(contractTicker) {
        try {
            ...
            const expParam = this.dateToYYYYMMDD(expiration);
    ...
    /**
     * Get all expiration dates (as JS Date objects) for the given ticker.
     * Uses ThetaData's /v2/list/expirations endpoint.
     * @param {string} ticker - Underlying symbol (e.g. "AAPL").
     * @returns {Promise<Date[]>} - Array of expiration Date objects.
     */
    async getExpirations(ticker) {
        try {
            const url = `${this.baseUrl}/v2/list/expirations`;
            const params = { root: ticker };
            const response = await axios.get(url, { params });
            const rawExpirations = response.data.response || [];
            
            // Convert each YYYYMMDD to a JS Date
            const expirations = rawExpirations.map(item => {
    ...
    /**
     * Get strikes for a given ticker and expiration date.
     * Uses ThetaData's /v2/list/strikes endpoint.
     * @param {string} ticker - Underlying symbol (e.g. "AAPL").
     * @param {Date} date - Expiration date as a JS Date object.
     * @returns {Promise<number[]>} - Array of strike prices as floats (e.g. 275.0).
     */
    async getStrikes(ticker, date) {
        try {
            const expParam = this.dateToYYYYMMDD(date);
            const url = `${this.baseUrl}/v2/list/strikes`;
            const params = {
                root: ticker,
                exp: expParam
            };
            ...
            const response = await axios.get(url, { params });
            const rawStrikes = response.data.response || [];
            
            // Convert raw strikes from integer to float by dividing by 1000
            return rawStrikes.map(strike => strike / 1000);
        } catch (error) {
            console.error('Error fetching strikes:', error.response?.data || error.message);
            return [];
        }
    }
}
...
(async () => {
    ...
    const expirations = await options.getExpirations('AAPL');
    console.log('Expirations:', expirations);

    // If we have at least one expiration, fetch strikes for it
    if (expirations.length > 0) {
        const strikes = await options.getStrikes('AAPL', expirations[0]);
        console.log('Strikes for first expiration:', strikes);
    }
